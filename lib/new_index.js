...
class Client extends EE {
	...
	async createServer(arg) {
		...
		/**return new Promise(function(resolve, reject) {
			client.addEventListener('newServer', function(server) {
				
			});
		});

		return this.requester.request('post', Endpoints.SERVERS(), payload);**/
		//Finish later
	}
	...
	async queryInvite(inviteCode) {
		return this.requester.request('get', Endpoints.INVITES(inviteCode))
...
class Server {
	constructor(client, data) {
		...
		this.afkChannel = this.channels[data.afk_channel_id] || null;
		data = null;

		Object.seal(this);
	}

	static toPayload(server) {
		return {
			name: server.name,
			icon: server.icon,
			region: server.region,
			splash: server.splash,
			afk_channel_id: server.afkChannel ? server.afkChannel.id : null,
			afk_timeout: server.afkTimeout,
			system_channel_id: server.systemChannel.id ? server.systemChannel.id : null,
			verification_level: server.verificationLevel,
			default_message_notifications: server.defaultMessageNotifications,
			explicit_content_filter: server.explicitContentFilter
		};
	}
	...
	/**
	 * Create a new role for this server.
	 * @param {Object} [arg]
	 * @param {String} [arg.name]
	 * @param {Number} [arg.color]
	 * @param {Boolean} [arg.hoist]
	 * @param {Boolean} [arg.mentionable]
	 */
	async createRole(arg) {
		if (typeof(arg) !== 'object')
			return this.client.requester.request('post', Endpoints.ROLES(this.id));

		return this.client.requester.request('post', Endpoints.ROLES(this.id), Role.verify_argument(arg));
	}
	 ...
	 */
	async getInvites() {
		return this.client.requester.request('get', `${Endpoints.SERVERS(this.id)}/invites`);
	}
}
class Channel {
	constructor(client, data) {
...
class User {
	...
	get avatarURL() {
		...
		var animated = this.avatarURL.indexOf("a_") > -1;
...
class Role {
	...
	static verify_argument(arg) {
		if (typeof(arg) !== 'object') throw new Error("Supplied argument must be an object");
		...
		if (typeof(arg.name) == 'string') payload.name = arg.name;
		if (typeof(arg.position) == 'number') payload.postion = arg.position;
		if (typeof(arg.color) == 'number') {
			payload.color = Math.max(Math.min(0xFFFFFF, arg.color), 0);
		}
		if (arg.permissions && arg.permissions.constructor == Permissions) {
			payload.permissions = arg.permissions.role_value.permissions;
		}
...
class Invite {
	constructor(client, data) {
		...
		this.channel = client.channels[data.channel.id] || null;
		this.inviter = client.users[data.inviter.id] || null;
	...
	async delete() {
		return this.client.requester.request(`delete`, Endpoints.INVITES(this.code));
	}
}
class Permissions {
	constructor(from) {
