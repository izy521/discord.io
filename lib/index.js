...
function DiscordClient(options) {
	...
	function handleWSMessage(data, flags) {
		...
		var serverID, channelID, userID,
			userItem, DMItem, chItem, userObjItem, userItemArray = ['status', 'game'],
			userObj, chObj,
			old;
		...
		switch (message.t) {
			...
			case "PRESENCE_UPDATE":
				...
				if (!self.users[userID]) self.users[userID] = message.d.user;
				if (!self.servers[serverID].members[userID]) {
					self.servers[serverID].members[userID] = {};
				}

				for (userItem in message.d) {
					if (userItemArray.indexOf(userItem) > -1) {
						self.users[userID][userItem] = message.d[userItem];
						continue;
					}
					if (userItem === 'user') {
						userObj = message.d[userItem];
						for (userObjItem in userObj) {
							self.users[userID][userObjItem] = userObj[userObjItem];
						}
						continue;
					}
					self.servers[serverID].members[userID][userItem] = message.d[userItem];
				}

				self.emit('presence', self.users[userID].username, message.d.user.id, message.d.status, message.d.game ? message.d.game.name : null , message);
				break;
					...
					if (userItem != "guild_id") {
						if (userItem === "user") {
							for (var uuItem in message.d[userItem]) {
								self.servers[serverID].members[message.d.user.id][userItem][uuItem] = message.d[userItem][uuItem];
							}
						} else {
							self.servers[serverID].members[message.d.user.id][userItem] = message.d[userItem];
						}
					}
				}
				self.emit('presence', self.servers[serverID].members[message.d.user.id].user.username, message.d.user.id, message.d.status, message.d.game ? message.d.game.name : null , message);*/
			case "USER_UPDATE":
				for (userItem in message.d) {
			...
			case "GUILD_UPDATE":
				...
				for (var key in message.d) {
					if (message.d[key] instanceof Array) {
						message.d[key].forEach(cpyGuildItem);
						continue;
					}
					self.servers[message.d.id][key] = message.d[key];
				}
				function cpyGuildItem(item) {
			...
			case "CHANNEL_CREATE":
				channelID = message.d.id;
				if (!self.channels[channelID]) self.channels[channelID] = message.d;
				...
				if (message.d.is_private === true) {
					...
					chObj = self.directMessages[message.d.id];
				} else {
					serverID = message.d.guild_id;
					if (self.servers[serverID].channels[channelID]) return;
					self.servers[serverID].channels[channelID] = {};
					self.servers[serverID].channels[channelID].members = {};
					chObj = self.servers[serverID].channels[channelID];
				}

				Object.keys(self.channels[channelID]).forEach(function(key) {
					Object.defineProperty(chObj, key, {
						get: function() { return self.channels[channelID][key]; }
					});
				});
				break;
			case "CHANNEL_UPDATE":
				...
				if (!self.channels[channelID]) self.channels[channelID] = {};
				old = self.channels[channelID];
				self.channels[channelID] = message.d;
				return emit(message, old, self.channels[channelID]);
			case "CHANNEL_DELETE":
				if (message.d.is_private === true) {
					emit(message, self.channels[message.d.id]);
					delete self.channels[message.d.id];
					return delete uIDToDM[message.d.recipient.id];
				}
				emit(message, self.channels[message.d.id]);
				return delete self.channels[message.d.id];
			...
			case "GUILD_MEMBERS_CHUNK":
				...
				return message.d.members.forEach(function(member) {
					var uID = member.user.id;
					if (self.servers[serverID].members[uID]) return;
					if (!self.users[uID]) self.users[uID] = member.user
					self.servers[serverID].members[uID] = new Member(member);
				});
		}
		return emit(message);
	}
		...
		(function changeVoiceChannel(channel, session) {
			...
			ws.once('message', function(m) {
				...
				if (
					...
					(m.d.guild_id !== vChannels[channel].guild_id)
	...
	function Server(data) {
		...
		data.channels.forEach(function(channel) {
			...
			channel.members = {};
			server.channels[channel.id] = new ServerChannel(channel)
		});
		data.members.forEach(function(member) {
			var uID = member.user.id;
			if (!self.users[uID]) self.users[uID] = member.user;
			server.members[uID] = new Member(member);
		});
		data.presences.forEach(function(presence) {
			for (var pkey in presence) {
				if (pkey !== 'user') {
					try { server.members[presence.user.id][pkey] = presence[pkey]; } catch(e) {}
				}
			}
		});
...
<<<<<<< HEAD
	...
	function ServerChannel(data) {
		var cID = data.id, channel = this;

		Object.keys(data).forEach(function(key) {
			Object.defineProperty(channel, key, {
				get: function() { return self.channels[cID][key]; },
				enumerable: true
			});
		});
	}
	...
	function Member(data) {
		var uID = data.user.id, member = this;
		for (var key in data) {
			this[key] = data[key];
=======
	function Channel(data) {}
	function User(data) {
		//Honestly don't think this should be a prototype
		//just yet, but whatever.
		return data;
	}
	function Member(data) {
		var self = this;
...
>>>>>>> parent of 00b72b4... Bugfixes (Member, audio)
		}
		delete(this.user);
		['id', 'game', 'avatar', 'status', 'username', 'discriminator'].forEach(function(key) {
			Object.defineProperty(member, key, {
				get: function() { return self.users[uID][key]; },
				enumerable: true
			});
		});
	}
