...
function DiscordClient(options) {
	...
	var needle = require('needle'),
		...
		ws, KAi, cv, uv, vChannels = {}, uIDToDM = {}, messageCache = {};
	/*Version check*/
	try {
		...
		function handleJoinVoice(vMessage) {
			...
			switch (vData.op) {
				...
				case 4:
					...
					return vWS.removeEventListener('message', handleJoinVoice);
			}
		}
	}
	...
	self.getAudioContext = function(channelObj, callback) { //Thanks #q/qeled.
		checkRS(function() {
			...
			if (!vChannels[channel]) return console.log("You have not joined the voice channel: " + channel);
			if (vChannels[channel].ready !== true) return console.log("The connection to the voice channel " + channel + " has not been initialized yet.");
			...
			vChannels[channel].udp.audio = vChannels[channel].udp.audio || new AudioCB(audioChannels, opusEncoder, udpClient, vWS, channel);
			...
			function AudioCB(audioChannels, opusEncoder, udpClient, vWS, channel) {
				...
				var self = this, streamRef,
					...
					VoicePacket = (function() { //Yay IIFEs :D
						...
						header[0] = 0x80;
						header[1] = 0x78;
						header.writeUIntBE(vChannels[channel].ws.ssrc, 8, 4);
				...
				try {
					port = vChannels[channel].ws.port;
					address = vChannels[channel].address;
					secretKey = vChannels[channel].secretKey;
				} catch(e) {return};
