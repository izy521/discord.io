...
function DiscordClient(options) {
		...
		checkRS(function() {
			function handleVoice(message) {
				...
				if (data.t === "VOICE_STATE_UPDATE") {
					...
					ws.removeListener('mesage', handleVoice);
				}
			}
			var server,
				token,
	...
	self.testAudio = function(input, callback) {
		checkRS(function() {
			function AudioCB(opusEncoder, udpClient, vWS) {
				require('stream').call(this);
				var self = this;
				var playingAF = false;
				var streamRef;
				var startTime;
				
				function sendAudio(sequence, timestamp, opusEncoder, streamOutput, udpClient, vWS, cnt) {
					cnt++;
					var buff;
					try {
				...
				this.playAudioFile = function(location) {
					...
					if (!playingAF) {
						...
						if (selection) {
							...
							enc.stdout.on('error', function(e) {
								enc.stdout.emit('end');
							});
							enc.stdout.once('readable', function() {
								vWS.send(JSON.stringify(speakingStart));
								startTime = new Date().getTime();
								sendAudio(0, 0, opusEncoder, enc.stdout, udpClient, vWS, 0);
							})
							streamRef = enc;
						}
					} else {
				...
				this.on('readable', function() { //If you're piping a stream, you can handle everything else.
					sendAudio(0, 0, opusEncoder, self, udpClient, vWS, 0);
				});
			}
			var channelID = input.channel, childProc, spawn, spawnSync, encs = ['ffmpeg', 'avconv'], enc, selection, vWS, udpClient, Opus, opusEncoder, speakingStart;
			...
			if (vChannels[channelID]) {
				try { Opus = require('node-opus'); } catch(e) { console.log("You need the Node module: 'node-opus'"); return false; }
					...
					if (vChannels[channelID].ready === true) {
						...
						util.inherits(AudioCB, require('stream'));
						callback(new AudioCB(opusEncoder, udpClient, vWS));
