...
function DiscordClient(options) {
	...
	self.setPresence = function(input) {
		checkRS(function() {
			...
			payload.d.game = (input.game ? {name: String(input.game)} : null)
	...
	self.editNickname = function(input, callback) {
		checkRS(function() {
			...
			req('patch', 'https://discordapp.com/api/guilds/' + input.serverID + '/members/' + input.userID, payload, function(err, res) {
	...
	function getToken() {
		...
		req('post', "https://discordapp.com/api/auth/login", {email: options.email, password: options.password}, function (err, res) {
			if (err || !checkStatus(res)) {
				...
				console.log(m + ": \n" + checkError(res));
				self.emit("disconnect", m);
				return self.emit("disconnected", m);
			}
	...
	function req(method, url) {
		return apiRequest.apply(self, arguments);
	}
	function type(n) {
...
function messageHeaders() {
	var r = {
		...
		"dnt": "1",
		"user-agent": "DiscordBot (https://github.com/izy521/discord.io, " + this.version + ")"
	};
