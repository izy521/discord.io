...
class DiscordClient extends (isNode ? EE : Object) {
	 ...
	 * @arg {String} input.message - An optional message to provide.
	 */
	uploadFile(input) {
		var multi, message;
	...
	getMessages(input) {
		...
		var total = typeof(input.amount) !== 'number' ? 50 : input.amount;
	 ...
	 */
	deleteMessages( input = {channelID: "", messageIDs: []} ) {
		return this._req('post', Endpoints.BULK_DELETE(input.channelID), {messages: input.messageIDs.slice(0, 100)});
	}
}
