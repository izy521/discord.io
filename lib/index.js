...
function DiscordClient(options) {
	...
	function getDirectMessages(DMArray) {
		for (var DM=0; DM<DMArray.length; DM++) {
			self.directMessages[DMArray[DM].id] = new DMChannel(self, uIDToDM, DMArray[DM])
		}
	}
	...
	function handleWSMessage(m, flags) {
		var message = flags.binary ? JSON.parse(zlib.inflateSync(m).toString()) : JSON.parse(m);
		var data = message.d;
		...
		switch (message.t) {
			...
			case "MESSAGE_CREATE":
				...
				emit(self, message, message.d.author.username, message.d.author.id, message.d.channel_id, message.d.content);
				return cacheMessage(messageCache, options.messageCacheLimit, message.d.channel_id, message.d);
			case "MESSAGE_UPDATE":
				try {
					emit(self, message, messageCache[message.d.channel_id][message.d.id], message.d);
				} catch (e) { emit(self, message, undefined, message.d); }
				return cacheMessage(messageCache, options.messageCacheLimit, message.d.channel_id, message.d);
			...
			case "GUILD_UPDATE":
				...
				return emit(self, old, self.servers[message.d.id]);
			case "GUILD_DELETE":
				emit(self, message, self.servers[message.d.id]);
			...
			case "GUILD_MEMBER_ADD":
				...
				return emit(self, message, self.servers[message.d.guild_id].members[message.d.user.id]);
			case "GUILD_MEMBER_UPDATE":
				if (!self.servers[message.d.guild_id].members[message.d.user.id]) {
					self.servers[message.d.guild_id].members[message.d.user.id] = {};
				}
				var member = self.servers[message.d.guild_id].members[message.d.user.id];
				...
				return emit(self, message, old, self.servers[message.d.guild_id].members[message.d.user.id]);
			case "GUILD_MEMBER_REMOVE":
				if (!self.servers[message.d.guild_id]) break;
				self.servers[message.d.guild_id].member_count -= 1;
				emit(self, message, self.servers[message.d.guild_id].members[message.d.user.id]);
			...
			case "GUILD_ROLE_UPDATE":
				...
				self.servers[message.d.guild_id].roles[message.d.role.id] = new Role(message.d.role);
				return emit(self, message, old, self.servers[message.d.guild_id].roles[message.d.role.id]);
			case "GUILD_ROLE_DELETE":
				emit(self, message, self.servers[message.d.guild_id].roles[message.d.role_id]);
			...
			case "CHANNEL_CREATE":
				...
				if (message.d.is_private) {
					...
					self.directMessages[channelID] = new DMChannel(self, uIDToDM, message.d);
				} else {
			...
			case "CHANNEL_UPDATE":
				...
				return emit(self, message, old, self.channels[message.d.id]);
			case "CHANNEL_DELETE":
				if (message.d.is_private === true) {
					emit(self, message, self.directMessages[message.d.id]);
					delete self.directMessages[message.d.id];
					return delete uIDToDM[message.d.recipient.id];
				}
				emit(self, message, self.servers[message.d.guild_id].channels[message.d.id]);
...
Object.defineProperty(Resource.prototype, "creation_time", {
	get: function() { return this.creation_time = (+this.id / 4194304) + 1420070400000; },
