(function(Discord){
	...
	/* --- Version Check --- */
	try {
		if (!isNode) throw Error( CURRENT_VERSION = CURRENT_VERSION + "-browser" );
		//Throw to stop from continuing, not a return since that wouldn't load the rest of the script.
	...
	function removeAllListeners(emitter, type) {
		...
		l = getEventListeners(emitter);
		if (type) {
			if (l[type]) {
				for (i=0; i<l[type].length; i++) {
					l[type][i].remove();
				}
			}
		}
	...
	function Emitter() {
		...
		this.on = function(eName, eFunc) {
			return this.addEventListener(eName, function(e) {
				return eFunc.apply(null, (e.detail || [e.data]));
			});
		}
		this.once = function(eName, eFunc) {
			return this.addEventListener(eName, function once(e) {
				eFunc.apply(null, (e.detail || [e.data]));
				return this.removeEventListener(eName, once);
			});
		}
