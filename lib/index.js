	...
	function handleWSMessage(data, flags) {
		...
		switch (message.t) {
			...
			case "GUILD_CREATE":
				...
				break;
			case "GUILD_UPDATE":
				if (!self.servers[_data.id]) self.servers[_data.id] = {};
				old = copy(self.servers[_data.id]);
				for (var key in _data) {
					if (_data[key] instanceof Array) {
						_data[key].forEach(function(item) {
							self.servers[_data.id][key][item.id] = item;
						});
						continue;
					}
					self.servers[_data.id][key] = _data[key];
				}
				return emit(self, old, self.servers[_data.id]);
			case "GUILD_DELETE":
				emit(self, message, self.servers[_data.id]);
				return delete self.servers[_data.id];
			case "GUILD_MEMBER_ADD":
				self.servers[_data.guild_id].members[_data.user.id] = {};
				for (userItem in _data) {
					self.servers[_data.guild_id].members[_data.user.id][userItem] = _data[userItem];
				}
				self.servers[_data.guild_id].member_count += 1;
				return emit(self, message, self.servers[_data.guild_id].members[_data.user.id]);
			case "GUILD_MEMBER_UPDATE":
				if (!self.servers[_data.guild_id].members[_data.user.id]) {
					self.servers[_data.guild_id].members[_data.user.id] = {};
				}
				var member = self.servers[_data.guild_id].members[_data.user.id];
			...
			case "GUILD_MEMBER_REMOVE":
				if (!self.servers[_data.guild_id]) break;
				self.servers[_data.guild_id].member_count -= 1;
			...
			case "GUILD_ROLE_CREATE":
				if (self.servers[_data.guild_id].roles[_data.role.id]) break;
				self.servers[_data.guild_id].roles[_data.role.id] = new Role(_data.role);
				break;
			case "GUILD_ROLE_UPDATE":
				old = self.servers[_data.guild_id].roles[_data.role.id];
				self.servers[_data.guild_id].roles[_data.role.id] = new Role(_data.role);
			...
			case "CHANNEL_DELETE":
				...
				return delete self.servers[_data.guild_id].channels[_data.id];
