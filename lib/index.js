...
function DiscordClient(options) {
		...
		checkRS(function() {
			...
			function _sendMessage(messageJSON, target) {
				...
				if (target in uIDToDM) return request.post({ //If they use a User ID and if there's a channel ID already matching the User ID, send it there.
					...
					if (err || !checkStatus(res)) return console.log("Unable to send (private) message: " + checkError(res, body));
					if (typeof(callback) === 'function') return callback(JSON.parse(body));
				});
	...
	self.testAudio = function(channelObj, callback) {
		console.log("testAudio has been deprecated changed to 'getAudioContext'.");
