...
function DiscordClient(options) {
	...
	function _joinVoiceChannel(server, channel, token, session, endpoint) { //Might remove, but let's try out some voice
		endpoint = endpoint.replace(":80", "");
		var vWS = vChannels[channel].connection = new WebSocket("wss://" + endpoint, null, {rejectUnauthorized: false});
			...
			switch (vData.op) {
				case 2:
					for (var vKey in vData.d) {
						vChannels[channel][vKey] = vData.d[vKey];
					}
					...
					udpClient.send(udpDiscPacket, 0, udpDiscPacket.length, vData.d.port, endpoint, function(err) { self.emit("err", err) });
					break;
			}
		});
		vWS.on('close', function() {
			...
			console.log("Voice websocket closed for channel: " + channel);
		});
	}
	...
	self.createRole = function(serverID) { //There's no input for the role ID and no response containing it, so you'll have to listen to the websocket message
		checkRS(function() {
			request.post({
				headers: messageHeaders(),
	...
	/*Voice, will relocate and possibly change*/
	self.joinVoiceChannel = function(channel) {
		checkRS(function() {
			var server,
				token,
				session,
					...
					if (!vChannels[channel]) {
						...
						ws.on('message', function(message) {
							...
							if (data.t === "VOICE_STATE_UPDATE") {
								...
								_joinVoiceChannel(server, channel, token, session, endpoint);
							}
	...
	self.leaveVoiceChannel = function(channel) {
		if (vChannels[channel]) {
			vChannels[channel].connection.close();
