...
DCP.getUser = function(input, callback) {
	if (!this.bot) return handleErrCB("This account is a 'user' type account, and cannot use 'getUser'. Only bots can use this endpoint.", callback);
 ...
 * @arg {Object} input
 * @arg {String|null} input.status - Used to set status, use Discord.Status enums.
 * @arg {Object|null} input.game - Used to set game information.
 * @arg {String|null} input.game.name - The name of the game.
 * @arg {Number|null} input.game.type - Streaming service, ignore for no service, use Discord.Stream enums.
 * @arg {String|null} input.game.url - A URL matching the streaming service you've selected, use Discord.URL functions.
 */
DCP.setPresence = function(input) {
	var payload = Payloads.STATUS(this, input);
	return ws_send(this._ws, payload);
};
