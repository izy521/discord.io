				...
				ws.on('message', function(data, flags) {
					...
					if (message.t) {			
						switch(message.t) {
							...
							case "CHANNEL_CREATE":
								if (d.recipient) {
									if (!(d.recipient.id in self.directMessages)) {
										getDirectMessages();
									}
								}
						}
		...
		checkRS(function() {
			...
			for (var server in self.servers) {
				if (target in self.servers[server].channels) {
					...
					console.log("The bot can't send a mesage to the specified ID.");
				}
			}
