...
DCP.removeFromRole = function(input, callback) {
	...
	try {
		roles = copy(this.servers[serverID].members[userID].roles);
		if (roles.indexOf(roleID) < 0) return handleErrCB(("Role " + roleID + " not found for user " + userID), callback);
...
function colorFromRole(server, member) {
	return member.roles.reduce(function(p, c) {
		var role = server.roles[c];
		return role.position > p[0] && role.color ? [role.position, role.color] : c;
	}, [-1, null])[1];
}
...
Member.update = function(server, data) {
	if (!server.members[data.user.id]) return server.members[data.user.id] = new Member(data);
