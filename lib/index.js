...
DCP.kick = function(input, callback) {
	var channelID = input.channelID || input.channel;
	var serverID = input.serverID || channelID ? this.channels[channelID].guild_id : null;
...
DCP.editRole = function(input, callback) {
	...
	try {
		role = new Role(this.servers[serverID].roles[roleID]);
		payload = {
			name: role.name,
...
DCP.deleteRole = function(input, callback) {
	...
	this._req('delete', "https://discordapp.com/api/guilds/" + serverID + "/roles/" + roleID, function(err, res) {
...
DCP.editNickname = function(input, callback) {
	var payload = {
		nick: input.nick ? String(input.nick) : ""
	};
