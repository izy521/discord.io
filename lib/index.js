 ...
 * @arg {Snowflake} input.serverID
 * @arg {Snowflake} input.userID
 * @arg {String} [input.reason] - The reason for the ban.
...
DCP.ban = function(input, callback) {
	...
	if (!input.reason) {
		input.reason = "Banned by " + bot.username;
	}

	this._req('put', Endpoints.BANS(input.serverID, input.userID) + (input.lastDays ? "?reason=" + input.reason + "&delete-message-days=" + input.lastDays : ""), function(err, res) {
 ...
 * @deprecated
 */
DCP.createInvite = function(NUL, callback) {
	var payload, client = this;
