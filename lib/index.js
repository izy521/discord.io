...
function DiscordClient(options) {
	...
	function startConnection(gateway) {
		...
		ws.on('message', handleWSMessage); //add token saving
		...
		if (message.t) {
			switch (message.t) {
				...
				case "PRESENCE_UPDATE":
					...
					self.emit('presence', self.servers[server].members[message.d.user.id].user.username, message.d.user.id, message.d.status, message.d.game.name, message);
					break;
				case "USER_UPDATE":
					if (message.d) {
						for (var userItem in message.d) {
