...
function DiscordClient(options) {
	...
	function getGateway(token, explicit) {
		...
		request.get("https://discordapp.com/api/gateway", function (err, res, gatewayBody) {
			if (err || !checkStatus(res)) {
				...
				if (explicit) return console.log("Incorrect login token provided.");
				return getToken();
			}
	...
	function generateMessage(channelID, message) {
		...
		return {
			...
			mention_everyone: message.indexOf('@everyone') > -1
		}
	}
	...
	function _joinVoiceChannel(server, channel, token, session, endpoint, callback) {
		...
		dns.lookup(endpoint, function(err, address, family) {
			...
			vWS.once('close', function() {
				try {self.leaveVoiceChannel(channel);} catch(e) {}
				clearInterval(vKAI);
			});
		});
	}
	...
	self.editUserInfo = function(input, callback) {
		checkRS(function() {
			...
			request.patch({
				...
				if (err || !checkStatus(res)) console.log("Error editing user info: " + checkError(res, body));
				try { body = JSON.parse(body) } catch(e) {};
				if (typeof(callback) === 'function') return callback(body);
			});
		});
	};
	...
	self.uploadFile = function(input, callback) { /* After like 15 minutes of fighting with Request, turns out Discord doesn't allow multiple files in one message... despite having an attachments array.*/
		checkRS(function() {
			var target = input.to;
			if (input.channel) {
				console.log("`uploadFile` uses 'to' now, instead of 'channel'. Please switch as 'channel' will be removed in the future.");
			...
			resolveID(target, function(channelID) {
				request.post({
					...
					if (err || !checkStatus(res)) console.log("Unable to upload file: " + checkError(res, body));
					try { body = JSON.parse(body) } catch(e) {};
					if (typeof(callback) === 'function') return callback(body);
				});
			});
			...
			function _sendMessage(messageJSON, target) {
				resolveID(target, function(channelID) {
					request.post({ //Finally, the ID must not belong to a User, so send the message directly to it, as it must be a channel.
						...
						if (err || !checkStatus(res)) console.log("Unable to send message: " + checkError(res, body));
						try { body = JSON.parse(body) } catch(e) {};
						if (typeof(callback) === 'function') return callback(body);
					});
				});
	...
	self.getMessages = function(input, callback) {
		checkRS(function() {
			...
			request.get({
				...
				if (err || !checkStatus(res)) console.log("Unable to get recent messages. " + checkError(res, body));
				try { body = JSON.parse(body) } catch(e) {};
				if (typeof(callback) === 'function') return callback(body);
			});
		});
	...
	self.editMessage = function(input, callback) {
		checkRS(function() {
			...
			request.patch({
				...
				if (err || !checkStatus(res)) console.log("Unable to edit message. " + checkError(res, body));
				try { body = JSON.parse(body) } catch(e) {};
				if (typeof(callback) === 'function') return callback(body);
			});
		});
	...
	self.createServer = function(input, callback) {
		checkRS(function() {
			var icon = input.icon === undefined ?  null : "data:image/png;base64," + input.icon;
			...
			request.get({
				...
				if (regions.indexOf(input.region) === -1) return console.log("You need to use one of these for regions:" + regions.map(function(rname) { return  " " + rname}));
				request.post({
					url: "https://discordapp.com/api/guilds",
					body: JSON.stringify(JSON.stringify({icon: icon, name: input.name, region: input.region}))
				}, function(err, res, body) {
					if (err || !checkStatus(res)) console.log("Could not create server: " + checkError(res, body));
					try {
						body = JSON.parse(body);
						self.servers[body.id] = {};
						for (var skey in body) {
							self.servers[body.id][skey] = body[skey];
						}
					} catch(e) {};
					if (typeof(callback) === 'function') return callback(body);
				});
		...
		checkRS(function() {
			request.get({
				...
				if (!err && checkStatus(res)) {
					...
					catch(e) { return console.log("Server regions request failed"); }
					var regions = body.map(function(region) { return region.id;	});
					var server = null;
					input = input || {};
					if (input.icon) input.icon = "data:image/jpg;base64," + input.icon;
					try {
						var resolvedServer = serverFromChannel(input.server);
						if (resolvedServer) server = resolvedServer;
					} catch(e) { return console.log("Server " + serverFromChannel(server) + " not found in cache. Is the client a member of the server?"); }
					if (!input.server) return console.log("Could not resolve server from " + input.server);
					var serverCache = self.servers[server];

					var payload = {
						name: input.name || serverCache.name,
						icon: input.icon || serverCache.icon,
	...
	self.acceptInvite = function(inviteCode, callback) {
		checkRS(function() {
			request.post({
				...
				if (err || !checkStatus(res)) console.log("The invite code provided (%s) is incorrect.", inviteCode);
				try {
					body = JSON.parse(body);
					if (!self.servers[body.guild.id]) {
						...
						console.log("Already joined server: " + body.guild.id);
					}
				} catch(e) {};
				if (typeof(callback) === 'function') return callback(body);
			});
	...
	self.queryInvite = function(inviteCode, callback) {
		checkRS(function() {
			request.get({
				...
				if (err || !checkStatus(res)) console.log("Unable to get invite information: " + checkError(res, body));
				try { body = JSON.parse(body) } catch(e) {};
				if (typeof(callback) === 'function') return callback(body);
			});
		});
	...
	self.createChannel = function(input, callback) {
		checkRS(function() {
			...
			request.post({
				...
				if (err || !checkStatus(res)) console.log("Could not create channel: " + checkError(res, body));
				try {
					body = JSON.parse(body);
					self.servers[body.guild_id].channels[body.id] = {};
					for (var ckey in body) {
						self.servers[body.guild_id].channels[body.id][ckey] = body[ckey];
					}
				} catch(e) {}
				if (typeof(callback) === 'function') return calback(body);
			});
		});
	};
			...
			try {
				...
				request.get({
					...
					if (!err && checkStatus(res)) {
						...
						console.log(checkError(res, body));
					}
				});
			}
	...
	self.createRole = function(serverID, callback) {
		checkRS(function() {
			request.post({
				...
				if (err || !checkStatus(res)) console.log("Could not create role: " + checkError(res, body));
				try {
					body = JSON.parse(body);
					self.servers[serverID].roles[body.id] = {};
					for (var rkey in body) {
						self.servers[serverID].roles[body.id][rkey] = body[rkey];
					}
				} catch(e) {}
				if (typeof(callback) === 'function') return callback(body);
			});
			...
			try {
				...
				request.patch({
					...
					if (!err && checkStatus(res)) {
						...
						console.log("Unabled to edit role: " + checkError(res, body));
					}
				});
									...
									request.patch({
										...
										if (!err && checkStatus(res)) {
											try {
												...
												if (pbody.message) {
													console.log("Could not set role: " + roleID + ", " + checkError(res, pbody));
												}
											} catch(e) {}
											if (typeof(callback) === 'function') {
												callback();
								...
								request.patch({
									...
									if (!err && checkStatus(res)) {
										try {
											pbody = JSON.parse(pbody);
											if (pbody.message) {
												console.log("Could not remove role: " + roleID + ", " + checkError(res, pbody));
											}
										} catch(e) {}
										if (typeof(callback) === 'function') {
											callback();
	...
	self.testAudio = function(channelObj, callback) {
		console.log("testAudio has been deprecated, please use 'getAudioContext'.");
		return self.getAudioContext(channelObj, callback);
	};
