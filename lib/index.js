	...
	self.editUserInfo = function(input, callback) {
		checkRS(function() {
			...
			request.patch({
				...
				if (err || !checkStatus(res)) return console.log("Error editing user info: " + checkError(res, body));
		...
		checkRS(function() {
			...
			resolveID(target, function(channelID) {
				request.post({
					...
					if (err || !checkStatus(res)) return console.log("Unable to upload file: " + checkError(res, body));
					if (typeof(callback) === 'function') return callback(JSON.parse(body));
				});
			});
		});
	...
	self.getMessages = function(input, callback) {
		checkRS(function() {
			...
			request.get({
				...
				if (err || checkStatus(res)) return console.log("Unable to get recent messages. " + checkError(res, body));
				if (typeof(callback) === 'function') return callback(JSON.parse(body));
			});
		});
	};
	self.editMessage = function(input, callback) {
		checkRS(function() {
			...
			request.patch({
				...
				if (err || !checkStatus(res)) return console.log("Unable to edit message. " + checkError(res, body));
				if (typeof(callback) === 'function') return callback(JSON.parse(body));
			});
		});
	};
	...
	self.acceptInvite = function(inviteCode, callback) {
		checkRS(function() {
			request.post({
				...
				if (err || !checkStatus(res)) return console.log("The invite code provided (%s) is incorrect.", inviteCode);
				if (typeof(callback) === 'function') return callback(JSON.parse(body));
			});
		});
	};
