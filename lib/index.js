...
function DiscordClient(options) {
	...
	self.testAudio = function(input) {
		checkRS(function() {
			...
			var cnt = 0;
			function sendAudio(sequence, timestamp, opusEncoder, fileOutput, udpClient, vWS) {
				cnt++;
				var buff;
				try {
					buff = fileOutput.read( 1920 );
					if (buff && buff.length === 1920) {
						...
						setTimeout(function() {
							sendAudio(sequence, timestamp, opusEncoder, fileOutput, udpClient, vWS);
			...
			var spawnSync = childproc.spawnSync;
			var encs = ['ffmpeg', 'avconv'];
			var selection = choosePlayer(encs);
		 	if (selection) {
				//try { require('fs').statSync(file); } catch(e) { console.log(e); return false; }
				var enc = spawn(selection , [
					'-i', file,
					'-f', 's16le',
					'-ar', '48000',
					'-ac', '1',
				...
				var vWS, udpClient, Opus;

				if (vChannels[channelID]) {
					   try {
						   Opus = require('node-opus');
					   } catch(e) {
						   console.log("You need the Node module: 'node-opus'");
						   return false;
					   }
					   ...
					   if (vChannels[channelID].ready === true) {
						   ...
						   enc.stdout.once('readable', function() {
							   sendAudio(sequence, timestamp, opusEncoder, enc.stdout, udpClient, vWS);
						   });
			...
			var type, audio, childProc, spawn, spawnSync, encs = ['ffmpeg', 'avconv'], selection, vWS, udpClient, Opus, opusEncoder, speaking;
			
			if (vChannels[channelID]) {
				if (!input.type) {
					console.log("There needs to be a type property, and it needs to be either 'file' or 'stream'");
					return false;
				} else {
					type = input.type;
					audio = input.audio;

					try { Opus = require('node-opus'); } catch(e) { console.log("You need the Node module: 'node-opus'"); return false; }
					opusEncoder = new Opus.OpusEncoder( 48000, 1 );
					speaking = {	"op":5,	"d":{	"speaking":true,	"delay":0	}	};
					...
					if (vChannels[channelID].ready === true) {
						...
						} else { console.log("The testAudio type has to be 'file' or 'stream'"); return false; }
					} else { self.emit('err', "The connection to the voice channel " + channelID + " has not been initialized yet."); }
				}
			} else { self.emit('err', "You have not joined the voice channel: " + channelID); }
