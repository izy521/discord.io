...
DCP.editMessage = function(input, callback) {
	...
	req('patch', "https://discordapp.com/api/channels/" + channelID + "/messages/" + input.messageID, generateMessage(channelID, input.message), function(err, res) {
		handleResCB("Unable to edit message", err, res, callback);
	});
};
DCP.deleteMessage = function(input, callback) {
	var channelID = input.channelID || input.channel;
	req('delete', "https://discordapp.com/api/channels/" + channelID + "/messages/" + input.messageID, function(err, res) {
...
DCP.editNote = function(input, callback) {
	//https://discordapp.com/api/users/@me/notes/
...
function apiRequest(method, url) {
	...
	if (type(data) === 'object') req.setHeader("Content-Type", "application/json; charset=utf-8");
	if (data instanceof Multipart) req.setHeader("Content-Type", "multipart/form-data; boundary=" + data.boundary);
	if (data) req.write( data.result || zlib.gzipSync(JSON.stringify(data)), data.result ? 'binary' : 'utf-8' );
	req.end();
