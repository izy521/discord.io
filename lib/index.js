...
function APIRequest(method, url) {
	...
	if (isNode) {
		...
		if (typeof(data) == 'object' || typeof(data) == 'array' || method.toLowerCase() === 'get') req.setHeader("Content-Type", "application/json; charset=utf-8");
	...
	if (typeof(data) == 'object' || typeof(data) == 'array' || method.toLowerCase() === 'get') req.setRequestHeader("Content-Type", "application/json; charset=utf-8");
...
function startConnection(client, gateway) {
	...
	client._ws.once('open', handleWSOpen.bind(client));
...
function handleWSOpen() {
	var ident = Payloads.IDENTIFY(this);
	this._connecting = false;
	if (this._shard) ident.d.shard = this._shard;
