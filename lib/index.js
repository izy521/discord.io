...
DCP.removeAllReactions = function(input, callback) {
	this._req('delete', Endpoints.REACTIONS(input.channelID, input.messageID), function(err, res) {
...
(function promisify() {
	//Promisify if available
	if (!Promise) return;
	return Object.keys(DCP).forEach(function(key) {
		if ( !(DCP[key] instanceof Function) ) return;
		if ( DCP[key].toString().indexOf('callback') < 0 ) return;

		return DCP[key + "P"] = function() {
			var arg = arguments[0];
			return new Promise(function(resolve, reject) {
				DCP[key].apply(this, (DCP[key].length === 1 ? [] : [arg]).concat( [function(err, res) {
...
function handleWSMessage(opts, data, flags) {
	...
	var _data = message.d;
	var client = this, userID, user, serverID, server, members, member, key, old, userItem, chItem;
...
function AudioCB(voiceSession, audioChannels, encoder, maxStreamSize) {
	...
	applyProperties(this, [
		...
		["_mixedDecoder", !Opus ? null : new Opus.OpusEncoder( 48000, audioChannels )],
	]);

	var enc = this._systemEncoder = ChildProc.spawn(encoder, [
		'-i', 'pipe:0',
		'-map', '0:a',
		'-acodec', 'libopus',
		'-f', 'data',
...
(function () {
	...
	Endpoints = Discord.Endpoints = {
		...
		REACTIONS: function(channelID, messageID) {
			return  this.MESSAGES(channelID, messageID) + "/reactions";
		},
		MESSAGE_REACTIONS: function(channelID, messageID, reaction) {
			return  this.REACTIONS(channelID, messageID)+ "/" + reaction;
		},
