...
DCP.leaveVoiceChannel = function(channelID, callback) {
	if (this._vChannels[channel]) return console.log("Not in the voice channel: " + channelID);
	if (this._vChannels[channel].ws || this._vChannels[channel].udp)
		return console.log("Currently connecting to voice channel: " + channelID + ", unable to leave until finished.");
...
function serverFromChannel(channel) {
	...
	for (var i=0; i<serverArray.length; i++) {
		if (this.servers[serverArray[i]].channels && this.servers[serverArray[i]].channels[channel]) return serverArray[i];
	}
}
function copy(obj) {
	try {
...
function leaveVoiceChannel(client, channelID, callback) {
	...
	client._vChannels[channel].udp.connection.close();
	send(client._ws, {
		op:4,
		d: {
...
function handleVoiceChannelChange(client, voiceSession) {
	...
	client._ws.once('message', function(m) {
		m = JSON.parse(m);
		if (
...
function AudioCB(voiceSession, opusEncoder, audioChannels) {
	...
	this.on('newListener', function(event) {
		if (isFinalIncomingEvListener(this, event)) {
			vUDP.on('message', handleMsgEvent);
		}
	});

	this.on('removeListener', function(event) {
		if (isFinalIncomingEvListener(this, event)) {
			vUDP.removeListener('message', handleMsgEvent);
		}
	});
}
...
ACBP.playAudioFile = function(location) {
	...
	enc = ChildProc.spawn(selection , [
		...
		'-f', 's16le',
		'-ar', '48000',
		'-ac', this._audioChannels,
		'pipe:1'
	], {stdio: ['pipe', 'pipe', 'ignore']});
	enc.stdout.once('end', function() {
		enc.kill();
		send(this._voiceSession.ws.connection, this._speakingEnd);
	...
	enc.stdout.once('readable', function() {
		...
		prepareAudio(this, enc.stdout, 1);
	});
	this._streamRef = enc;
};
...
function prepareAudio(ACBI, readableStream) {
	...
	readableStream.on('end', function() {
		...
		send(ACBI._voiceSession.ws.connection, ACBP.prototype._speakingEnd);
	});
...
function sendAudio(ACBI, buffer) {
	...
	var audioPacket = VoicePacket(buffer, ACBI._voiceSession.ws.ssrc, ACBI._sequence, ACBI._timestamp, ACBI._voiceSession._secretKey);
