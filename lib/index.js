...
function DiscordClient(options) {
	...
	function handleWSMessage(data, flags) {
		...
		switch (message.t) {
			...
			case "PRESENCE_UPDATE":
				...
				self.emit('presence', self.servers[serverID].members[userID].user.username, message.d.user.id, message.d.status, message.d.game ? message.d.game.name : null , message);
				break;
	...
	function Server(data) {
		...
		data.channels.forEach(function(channel) {
			...
			Object.keys(self.channels[channel.id]).forEach(function(key) {
				Object.defineProperty(channel, key, {
					get: function() { return self.channels[channel.id][key]; }
				});
			});
	...
	function Member(data) {
		...
		Object.defineProperties(this, {
			id: { get: userProp(uID, 'id') },
			game: { get: userProp(uID, 'game') },
			avatar: { get: userProp(uID, 'avatar') },
			status: { get: userProp(uID, 'status') },
			username: { get: userProp(uID, 'username') },
			discriminator: { get: userProp(uID, 'discriminator') },
		});
	}
	function userProp(uID, prop) {
		return function() {
			return self.users[uID][prop];
		}
	}
