...
function DiscordClient(options) {
	...
	function handleResCB(errMessage, err, res, body, callback) {
		if (typeof(callback) !== 'function') return;
		if (err || !checkStatus(res)) return callback(errMessage + ": " + checkError(res, body));
	...
	self.uploadFile = function(input, callback) { /* After like 15 minutes of fighting with Request, turns out Discord doesn't allow multiple files in one message... despite having an attachments array.*/
		checkRS(function() {
			var target = input.to, file = input.file;
			var formData = {
				file: file
			};
	...
	self.editMessage = function(input, callback) {
		checkRS(function() {
			request.patch({
				...
				body: JSON.stringify(generateMessage(channelID, input.message))
			}, function(err, res, body) {
	...
	self.kick = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== "object") return console.log("The kick method requires an object argument");
	...
	self.ban = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== "object") return console.log("The ban method requires an object argument");
	...
	self.unban = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== "object") console.log("The unban method requires an object argument");
	...
	self.moveUserTo = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== 'object') return console.log("The moveUserTo method requires an object argument");
			request.patch({
	...
	self.mute = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== "object") return console.log("The mute method requires an object argument");
	...
	self.unmute = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== "object") return console.log("The unmute method requires an object argument");
	...
	self.deafen = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== "object") return console.log("The defean method requires an object argument");
	...
	self.undeafen = function(input, callback) {
		checkRS(function() {
			if (typeof(input) !== "object") return console.log("The undeafen method requires an object argument");
		...
		checkRS(function() {
			...
			try {
				...
				for (var key in input) {
					if (Object.keys(payload).indexOf(key) === -1) continue;
					if (key === 'permissions') {
						for (var perm in input[key]) role.permission_values[perm] = input[key][perm];
						continue;
					}
					if (key === 'color') {
						if (String(input[key])[0] === '#') role.color = parseInt(String(input[key]).replace('#', '0x'), 16);
						if (role.color_values[input[key]]) role.color = role.color_values[input[key]];
						continue;
					}
					role[key] = input[key];
				}
		...
		checkRS(function() {
			...
			try {
				...
				request.patch({
					...
					handleResCB("Could not add role", er, res, body, callback);
				});
			} catch(e) {return handleErrCB(e, callback)}
		});
	};
	...
	self.joinVoiceChannel = function(channel, callback) {
		checkRS(function() {
			...
			if (!serverFromChannel(channel)) return handleErrCB(("Cannot find the server related to the channel provided: " + channel), callback);
