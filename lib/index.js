(function discordio(Discord){
	...
	DCP.sendMessage = function(input, callback) {
		var client = this, time, message = generateMessage(input.message);
	...
	DCP.editServer = function(input, callback) {
		var regions = ['amsterdam', 'brazil', 'frankfurt', 'london', 'singapore', 'sydney', 'us-central', 'us-east', 'us-south', 'us-west'],
			payload, serverID = input.serverID || input.server, server, client = this;
		if (input.icon) input.icon = "data:image/jpg;base64," + input.icon;
		...
		for (var key in input) {
			if (Object.keys(payload).indexOf(key) === -1) continue;
			if (key === 'region') {
				if (regions.indexOf(input[key]) > -1) payload[key] = input[key];
				continue;
			}
	...
	DCP.getAudioContext = function(channelObj, callback) {
		...
		if (!Opus) try { Opus = require('node-opus'); } catch(e) { return handleErrCB("Error requiring module: 'node-opus'"); }
	...
	function handleIncomingAudio(msg) {
		var header = msg.slice(0, 12),
			...
			enc = this._speakers[ssrc] || new Opus.OpusEncoder( 48000, 1 * this._audioChannels ),
			decrypted;
