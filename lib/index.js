(function discordio(Discord){
	...
	DCP.getAudioContext = function(channelObj, callback) {
		...
		voiceSession.maxStreamSize = Math.abs(Number(channelObj.maxStreamSize));
		voiceSession.audio = voiceSession.audio || new AudioCB(voiceSession, (channelObj.stereo === false ? 1 : 2));
	...
	function AudioCB(voiceSession, audioChannels) {
		...
		//repetition.
		var ACBI = this, 
		bHandleIncomingAudio = handleIncomingAudio.bind(this), 
		encoder, enc;
		...
		applyProperties(this, [
			...
			["_mixedDecoder", new Opus.OpusEncoder( 48000, 2 )]
		]);
		...
		try {
			...
			this._secretKey = new Uint8Array(voiceSession.secretKey);
		} catch(e) {return handleErrCB(e, voiceSession.callback); }

		encoder = chooseAudioEncoder(['ffmpeg', 'avconv']);
		if (!encoder) return handleErrCB("You need either 'ffmpeg' or 'avconv' and they need to be added to PATH", voiceSession.callback);
			...
			voiceSession.ws.connection.on('message', function(data, flags) {
				...
				if (!voiceSession.members[data.d.user_id]) {
					voiceSession.members[data.d.user_id] = new Stream.Readable({
						highWaterMark: voiceSession.maxStreamSize,
						read: function(s) {}
					});
	...
	function addToStreamBuffer(RStream, data) {
		while (data.length + RStream._readableState.length > RStream._readableState.highWaterMark) {
			RStream.read(data.length);
		}
		return RStream.push(new Buffer(data));
	}
