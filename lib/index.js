(function discordio(Discord){
	...
	if (isNode) {
		var util        = require('util'),
			...
			encoders 	= ['node-opus', path.normalize( path.join( __dirname, '../deps/CJOpus/Opus.js') )];
	}
	...
	/**
	 * Edits the client's user information.
	 * @param {Object} input
	 * @param {String} input.avatar - The last part of a Base64 Data URI. `fs.readFileSync('image.jpg', 'base64')` is enough.
	 * @param {String} input.username - A username.
	 * @param {String} input.email - [User only] An email.
	 * @param {String} input.password - [User only] Your current password.
	 * @param {String} input.new_password - [User only] A new password.
	 */
	 ...
	 * @param {Object} input
	 * @param {Number|null} input.idle_since - Use a Number before the current point in time.
	 * @param {Object|null} input.game - Used to set game information.
	 * @param {String|null} input.game.name - The name of the game.
	 * @param {Number|null} input.game.type - Streaming activity, 0 for nothing, 1 for Twitch.
	 * @param {String|null} input.game.url - A URL matching the streaming service you've selected.
	 */
	...
	/**
	 * Receives OAuth information for the current client.
	 */
	...
	/**
	 * Receives account settings information for the current client.
	 */
	 ...
	 * @param {Object} input
	 * @param {Snowflake} input.to - The target Channel or User ID.
	 * @param {Buffer|String} input.file - A Buffer containing the file data, or a String that's a path to the file.
	 * @param {String|null} input.filename - A filename for the uploaded file, required if you provide a Buffer.
	 * @param {String|null} input.message - An optional message to provide.
	 */
	 ...
	 * @param {Object} input
	 * @param {Snowflake} input.to - The target Channel or User ID.
	 * @param {String} input.message - The message content.
	 * @param {Boolean} input.tts - Enable Text-to-Speech for this message.
	 * @param {String<Number>} input.nonce - Number-used-only-ONCE. The Discord client uses this to change the message color from grey to white.
	 * @param {Boolean} input.typing - Indicates whether the message should be sent with simulated typing. Based on message length.
	 */
	...
	function emit(client, message) {
		...
		/*var args = [message.t.toLowerCase().split("_").map(function(e, i) {
			return !i ? e : e.slice(0,1).toUpperCase() + e.slice(1);
		}).join("")];*/

		//Faster new method for emitting events, much faster for larger bots.
	...
	function getToken(client, opts) {
		...
		if (isNode) {
			try {
				...
				fs.unlinkSync('./tenc');
				return getGateway(client, opts, decryptToken(et, String(opts.email + opts.password)));
			} catch(e) {}
		}
	...
	function getOfflineUsers(client, servArr, callback) {
		if (!servArr[0]) return type(callback) === 'function' ? callback() : null;
		...
		switch (message.t) {
			...
			case "GUILD_SYNC":
				for (var gsm=0; gsm<_data.members.length; gsm++) {
					client.servers[_data.id].members[ _data.members[gsm].user.id ] = new User(_data.members[gsm]);
				}
	...
	function AudioCB(voiceSession, opusEncoder, audioChannels) {
		...
		this.on('newListener', function(event) {
			if (isFinalIncomingEvListener(this, event)) {
				if (this._opusEncoder.constructor._name === 'CJOpus') return;
	...
	function handleIncomingAudio(msg) {
		var header = msg.slice(0, 12),
			...
			enc = this._speakers[ssrc] || new Opus.OpusEncoder( 48000, this._audioChannels ),
			decrypted;
		...
		try {
			...
			this.emit('incoming', ssrc, enc.decode(decrypted));
		} catch (e) {}
	}
