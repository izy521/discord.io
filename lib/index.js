 ...
 * @arg {Number|null} input.idle - Optional, use a Number before the current point in time.
...
DCP.setPresence = function(input) {
	...
	if (payload.d.idle === null) return void(this.presenceStatus = payload.d.status);
	...
	Payloads = {
		...
		STATUS: function(input) {
			return {
				op: 3,
				d: {
					status: type(input.idle) === 'number' ? 'idle' : input.status !== undefined ? input.status : null,
					afk: !!input.afk,
					since: type(input.idle) === 'number' || input.status === 'idle' ? Date.now() : null,
