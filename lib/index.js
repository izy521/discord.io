...
DCP.editServerEmoji = function(input, callback) {
	if ( !(input in name) ) return handleErrCB("[editServerEmoji] You need to supply a new Emoji name", callback);
	this._req('patch', Endpoints.SERVER_EMOJIS(input.serverID, input.emojiID), {name: input.name}, function(err, res) {
...
function handleWSMessage(opts, data, flags) {
	...
	switch (message.t) {
		...
		case "GUILD_EMOJIS_UPDATE":
			...
			Server.updateEmojis(client.servers[_data.guild_id], _data.emojis);
...
Server.updateEmojis = function(server, emojis) {
	server.emojis = {};
