					...
					if (!err && checkStatus(res)) {
						...
						request.get({
							...
							if (!err && checkStatus(res)) {
								...
								console.log("Error GETing client settings: " + res.statusMessage  + "\n" + body);
							}
						});

					} else {
						console.log("Error GETing gateway list: " + res.statusMessage  + "\n" + body);
						self.emit("disconnected");
						return false;
					}
				});
		...
		checkRS(function() {
			...
			request.patch({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Error editing user info: " + res.statusMessage  + "\n" + body);
				}
			});
		});
	};
		...
		checkRS(function() {
			...
			request.post({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Unable to upload file: " + res.statusMessage  + "\n" + body);
				}
			});
		});
	};
				...
				if (self.directMessages[target]) {
					request.post({
						...
						if (!err && checkStatus(res)) {
							...
							console.log("Unable to send message: " + res.statusMessage  + "\n" + body);
						}
					});
					return;
				} else {
		...
		checkRS(function() {
			...
			request.get({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Unable to get recent messages. " + res.statusMessage  + "\n" + body);
				}
			});
		});
	};
		...
		checkRS(function() {
			...
			request.patch({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Unable to edit message. " + res.statusMessage  + "\n" + body);
				}
			});
		});
	};
									...
									request.patch({
										...
										if (!err && checkStatus(res)) {
											try {
												...
												if (pbody.message) {
													console.log("Could not set role: " + roleID + ", " + res.statusMessage  + "\n" + body);
												}
											} catch(e) {}
											if (typeof(callback) === 'function') {
												callback();
								...
								request.patch({
									...
									if (!err && checkStatus(res)) {
										try {
											pbody = JSON.parse(pbody);
											if (pbody.message) {
												console.log("Could not remove role: " + roleID + ", " + res.statusMessage  + "\n" + body);
											}
										} catch(e) {}
										if (typeof(callback) === 'function') {
											callback();
