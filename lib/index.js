...
function DiscordClient(options) {
					...
					if (!err && checkStatus(res)) {
						...
						request.get({
							...
							if (!err && checkStatus(res)) {
								...
								console.log("Error GETing client settings: " + checkError(res, body));
							}
						});

					} else {
						console.log("Error GETing gateway list: " + checkError(res, body));
						self.emit("disconnected");
						return false;
					}
				});
	...
	/*function getToken() { //Building blocks, FOR A NEW GENERATION.
		if (input.token) return getGateway(token, true);
	}
	function getGateway(token, mod) {
	...
	function messageHeaders() {
		var headers = {
			...
			"user-agent": "DiscordBot (https://github.com/izy521/discord.io, " + (cv || "1.x.x") + ")"
		};
		return headers;
	}
	...
	function checkError(response, body) {
		return response.statusMessage ? response.statusCode + " " + response.statusMessage + "\n" + body : response.statusCode + "\n" + body;
	}
		...
		checkRS(function() {
			...
			request.patch({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Error editing user info: " + checkError(res, body));
				}
			});
		});
	};
		...
		checkRS(function() {
			...
			request.post({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Unable to upload file: " + checkError(res, body));
				}
			});
		});
	};
	self.sendMessage = function(input, callback) {
        checkRS(function() {
            ...
            var typingSpeed = speeds[input.speed] || 0.09;
            var typingTime = (input.message.length * typingSpeed) * 1000;
            var messageJSON;

            if (input.nonce) message.nonce = input.nonce;
            if (input.maxTypingTime) { typingTime = Math.min(input.maxTypingTime, typingTime); }

            message.tts = input.tts || false;
            messageJSON = JSON.stringify(message);
            input.typing = input.typing || false;
		...
		checkRS(function() {
			...
			request.get({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Unable to get recent messages. " + checkError(res, body));
				}
			});
		});
	};
		...
		checkRS(function() {
			...
			request.patch({
				...
				if (!err && checkStatus(res)) {
					...
					console.log("Unable to edit message. " + checkError(res, body));
				}
			});
		});
	};
	...
	self.testAudio = function(channelObj, callback) { //Thanks #q/qeled.
		checkRS(function() {
