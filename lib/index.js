...
function DiscordClient(options) {
	...
	self.fixMessage = function(message) {
		...
		var channels = message.match(/<#.*?>/g);
		if (mentions !== null) {
			mentions.forEach(function(id, index) {
				id = id.replace(/</g, "").replace(/@/g,"").replace(/>/g,"");
				var name;
				for (var server in self.servers) {
					if (self.servers[server].members[id]) {
						name = self.servers[server].members[id].user.username;
						break;
					}
				}
				message = message.replace(mentions[index], "@" + name);
			});
		}

		if (channels !== null) {
			channels.forEach(function(cID, index) {
				cID = cID.replace(/</g, "").replace(/#/g,"").replace(/>/g,"");
				var cName;
				for (var server in self.servers) {
					if (self.servers[server].channels[cID]) {
						cName = self.servers[server].channels[cID].name;
						break;
					}
				}
				message = message.replace(channels[index], "#" + cName);
			});
		}
		...
		/*var mentions = message.match(/<@.*?>/g); //Will remove later
        if (mentions !== null) {
	...
	self.testAudio = function(input) {
        checkRS(function() {
            console.log("As the name states, \"testAudio\" is a test/experimental.");
            function sendAudio(sequence, timestamp, opusEncoder, wavOutput, udpClient, vWS, speakingPacket, sPInterval) {
            ...
            if (vChannels[channelID]) {
                ...
                if (vChannels[channelID].ready === true) {
                    ...
                    //var mp3Decoder = new Lame.Decoder(); MP3
