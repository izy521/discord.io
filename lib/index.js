...
function DiscordClient(options) {
	...
	function getOfflineUsers(servID) {
		/*Object.keys(self.servers).filter(function(serverID) {
	...
	function handleWSMessage(data, flags) {
		...
		switch (message.t) {
			...
			case "GUILD_DELETE":
				emit(message, bot.servers[message.d.id]);
			...
			case "GUILD_MEMBERS_CHUNK":
				...
				var all = Object.keys(self.servers).every(function(s) {
					s = self.servers[s];
					return s.member_count === Object.keys(s.members).length;
				});

				if (all) return (self.emit('ready', ready), ready = undefined);
				return;
		}
	...
	function joinVoiceChannel(server, channel, token, session, endpoint, callback) {
		...
		var vKAI, vDiscIP = "", vDiscPort, vWS, udpClient;
		...
		dns.lookup(endpoint, function(err, address, family) {
			...
			vWS.on('message', handleJoinVoice);
			vWS.once('close', function() {
				clearInterval(vKAI);
			});
		});

		function handleJoinVoice(vMessage) {
			...
			switch (vData.op) {
				...
				case 4:
					...
					return vWS.removeListener('message', handleJoinVoice);
			}
		}
		(function changeVoiceChannel(channel, session) {
			...
			ws.once('message', function(m) {
				...
				if (m.d.channel_id == null) return leaveVoiceChannel(channel) //Change to proper internal leave
	...
	function Server(data) {
		...
		if (self.large) getOfflineUsers(self.id);
	...
	self.getAudioContext = function(channelObj, callback) {
		...
		checkRS(function() {
			...
			var vWS, udpClient, Opus, opusEncoder, speakingStart;
			...
			client.udp.audio = client.udp.audio || new AudioCB(audioChannels, opusEncoder, udpClient, vWS, channel);
			callback(client.udp.audio);

			function AudioCB(audioChannels, opusEncoder, udpClient, vWS, channel) {
				EE.call(this);
