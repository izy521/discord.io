									...
									case "VOICE_STATE_UPDATE":
										if (message.d) {
											if (message.d.channel_id !== null) {
												...
												try {
													//Discord doesn't emit the channel_id, and I don't want to for loop the voice channels that the user
													//could have been in.
	...
	self.testAudio = function(channelObj, callback) {
		checkRS(function() {
			...
			function AudioCB(audioChannels, opusEncoder, udpClient, vWS) {
				...
				function sendAudio(sequence, timestamp, audioChannels, opusEncoder, streamOutput, udpClient, vWS, cnt) {
					cnt++;
					var buff;
					try {
						...
						if (buff && buff.length === 1920 * audioChannels) {
							...
							var encoded = opusEncoder.encode(buff);
							...
							return setTimeout(function() {
								return sendAudio(sequence, timestamp, audioChannels, opusEncoder, streamOutput, udpClient, vWS, cnt);
							}, 20 + (nextTime - new Date().getTime()));
						} else {
							buff.length //This is there to trigger the try catch exception.
						}
					} catch (e) {
						console.log(e);
					...
					if (!playingAF) {
						...
						if (selection) {
							...
							enc.stdout.once('readable', function() {
								...
								sendAudio(0, 0, audioChannels, opusEncoder, enc.stdout, udpClient, vWS, 0);
							});
							//enc.stderr.on('data', function(d) { return d.toString(); }); //Possible fix (or workaround) to get around that 10 minute bug or possible ffmpeg encoding errors.
							streamRef = enc;
						}
					} else {
				...
				this.send = function(stream) { //If you're piping a stream, you can handle everything else.
					vWS.send(JSON.stringify(speakingStart));
					startTime = new Date().getTime();
					sendAudio(0, 0, audioChannels, opusEncoder, stream, udpClient, vWS, 0);
				}
			}
			...
			if (vChannels[channel]) {
				...
				if (vChannels[channel].ready === true) {
					...
					callback(new AudioCB(audioChannels, opusEncoder, udpClient, vWS));
				} else { self.emit('err', "The connection to the voice channel " + channel + " has not been initialized yet."); }
			} else { self.emit('err', "You have not joined the voice channel: " + channel); }
