...
self.listPins = function(channelID, callback) {
		checkRS(function() {
			req('get', "https://discordapp.com/api/channels/" + channelID + "/pins", function(err, res) {
				handleResCB('Unable to get pins list' + channelID, err, res, callback);
		});
	});
};
	
self.addPins = function(input, callback) {
		checkRS(function() {
			req('put', "https://discordapp.com/api/channels/" + input.channel + "/pins/" + input.messageID, function(err, res) {
				handleResCB("Unable to add pin", err, res, callback);
		});
	});
};
	
self.deletePins = function(input, callback) {
		checkRS(function() {
			req('delete', "https://discordapp.com/api/channels/" + input.channel + "/pins/" + input.messageID, function(err, res) {
				handleResCB("Unable to delete pin", err, res, callback);
		});
	});
};
