							...
							if (message.t) {
								switch (message.t) {
									...
									case "VOICE_STATE_UPDATE":
										var cid = message.d.channel_id;
										if (message.d) {
											if (cid !== null) {
												if (self.servers[message.d.guild_id].channels[cid]) {
		...
		checkRS(function() {
			...
			function AudioCB(audioChannels, opusEncoder, udpClient, vWS) {
				...
				function sendAudio(sequence, timestamp, audioChannels, opusEncoder, streamOutput, udpClient, vWS, cnt) {
					cnt++;
					var buff;
					buff = streamOutput.stdout.read( 1920 * audioChannels );
					if (!streamOutput.stdout.destroyed) {
						sequence + 1 < 65535 ? sequence += 1 : sequence = 0;
						timestamp + 960 < 4294967295 ? timestamp += 960 : timestamp = 0;
					...
					if (!playingAF) {
						...
						if (selection) {
							...
							enc.stdout.once('readable', function() {
								...
								sendAudio(0, 0, audioChannels, opusEncoder, enc, udpClient, vWS, 0); //used to be enc.stdout
							});
							streamRef = enc;
						}
					} else {
